...
function Login() {
  ...
  const handleConnect = async () => {
    try{
      ...
      if (typeof window !== 'undefined') {
        const chainId = await window.fire.request({ method: 'eth_chainId' })
        if (typeof window !== 'undefined') {
  ...
  const styles = {
    main_box: `bg-high_contrast_yellow border-2 w-full drop-shadow-3xl border-4 flex flex-col justify-around p-10`,
    input_bg: `h-12 mt-5 mb-5 flex justify-between`,
    small_input_box: `bg-white border-2 drop-shadow-2xl w-5/12 h-full`,
    connect_wallet: ` bg-dark_pink drop-shadow-2xl w-5/12 h-full flex flex-col justify-center items-center active:mt-2 active:drop-shadow-xl active:bg-grinish-yellow`,
